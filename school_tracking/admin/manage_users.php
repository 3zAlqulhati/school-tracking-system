...
if (!empty($userIds)) {
    ...
    $stmtStuD = $pdo->prepare(
        "SELECT s.user_id,
                (SELECT c.class_name    FROM student_classes sc
                  ...
                  WHERE sc.student_id = s.student_id LIMIT 1) AS class_name,
                (SELECT c.academic_year FROM student_classes sc
                  JOIN classes c ON c.class_id = sc.class_id
                  WHERE sc.student_id = s.student_id LIMIT 1) AS academic_year,
    ...
    $stmtParD = $pdo->prepare(
        "SELECT p.user_id, u2.full_name AS child_name, p.relationship,
                (SELECT c.class_name FROM student_classes sc
                  JOIN classes c ON c.class_id = sc.class_id
                  WHERE sc.student_id = s.student_id LIMIT 1) AS class_name
         FROM parents p
