...
$allStudentsForSelect = $pdo->query(
    "SELECT s.student_id, u.user_id, u.full_name, c.class_name, s.class_id
     FROM students s
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'send_notification') {
    ...
    if ($title === '' || $message === '') {
        ...
        switch ($recepType) {
            ...
            case 'class_students':
                $stmtR = $pdo->prepare(
                    'SELECT u.user_id FROM students s JOIN users u ON u.user_id = s.user_id WHERE s.class_id = ?'
                );
